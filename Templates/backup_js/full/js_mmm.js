...
if(browser=="NS6")
{if(version==6.1){top=x466187111(name)+x466187111(nm2+'clip');x483140(nm1,top);}
...
if(left+x57358931(nm1)>x56568364953685())
	{x3329217(nm1,x56568364953685()-x57358931(nm1));}
if(top<x632766380818())
	{x483140(nm1,x632766380818());}
if(top+x516306672(nm1)>x616803111554122())
	{x483140(nm1,x616803111554122()-x516306672(nm1));}x2826022670(nm1,true);}ajm_state.menuActive=true;};
function subMenuMouseOut(name,count,bgcolor,fgcolor)
	{if(browser!="Opera"){x2657480876(name+'itm'+count,bgcolor,name);x2498849083(name+'a'+count,fgcolor,name);}ajm_state.timeoutidArray[ajm_state.tiac++]=setTimeout("x432327267()",ajm_state.closedelay);};function addItems(){for(var i=0;i<arguments.length;i+=2){this.addItem(arguments[i],arguments[i+1]);}};

function x2080628753()
	{for(var i=0;i<arguments.length;i++){this.x2080628753(arguments[i]);}};
function addImages()
	{for(var i=0;i<arguments.length;i+=3){this.addItem(null,arguments[i],null,arguments[i+1],arguments[i+2]);}};
function addImagesWithTarget()
	{for(var i=0;i<arguments.length;i+=4){this.addItem(null,arguments[i],arguments[i+1],arguments[i+2],arguments[i+3]);}};
function addItemsWithTarget()
	{for(var i=0;i<arguments.length;i+=3){this.addItem(arguments[i],arguments[i+1],arguments[i+2]);}};
function x432327267()
	{if(!ajm_state.menuActive){for(var i=0;i<ajm_state.amac;i++){x2826022670(ajm_state.activemenuArray[i],false);}ajm_state.amac=0;}};
function displayMenu()
	{for(var i=ajm_state.mmcp;i<ajm_state.mmac;i++){ajm_state.mainmenuArray[i].writeMenu();}ajm_state.mmcp=ajm_state.mmac;};
function createMenu()
	{for(var i=ajm_state.smcp;i<ajm_state.smac;i++){ajm_state.submenuArray[i].writeMenu();}ajm_state.smcp=ajm_state.smac;};
function loadMenu()
	{if(os=="Mac"&&browser=="IE"){ajm_state.mtopoffset=document.body.topMargin;ajm_state.mleftoffset=document.body.leftMargin;}
...
function x56568364953685()
	{if(browser=="NS4"||browser=="NS6"||browser=="Opera"){return(window.innerWidth+window.pageXOffset);}if(browser=="IE"){return(document.body.clientWidth+document.body.scrollLeft);}};
function x5597204602725()
	{if(browser=="NS4"||browser=="NS6"||browser=="Opera"){return(window.pageXOffset);}
if(browser=="IE"){return(document.body.scrollLeft);}};
function x632766380818()
	{if(browser=="NS4"||browser=="NS6"||browser=="Opera"){return(window.pageYOffset);}if(browser=="IE"){return(document.body.scrollTop);}};
function x616803111554122()
	{if(browser=="NS4"||browser=="NS6"||browser=="Opera"){return(window.innerHeight+window.pageYOffset);}
