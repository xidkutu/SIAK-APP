/*
 Milonic DHTML Website Navigation Menu - Version 3.4.07
...
function gmobj(mtxt){if(_d.getElementById){m=_d.getElementById(mtxt)}else if(_d.all){m=_d.all[mtxt]}else if(_d.layers){m=_d.eval(mtxt)}return m;}
...
function sm(v1){menu=eval(v1);if(menu[19]&&!menu[21][2])fixb(v1);if(opera||ns6||menu[11]||IEDtD)if(!menu[7]&&!menu[21][2])fixb(v1);ap=gmobj(v1);if(flta)if(ap.filters[0])ap.filters[0].Apply();gmi++;if(!ns4)aps=ap.style; else aps=ap;aps.zIndex=gmi;aps.visibility='visible';if(flta)if(ap.filters[0])ap.filters[0].Play();if(menu[17])SDiv(menu[17],0)}
...
{
	if(MLoaded==0||inDragMode)return;
...
{		
	...
	subimg="<img id="+ims+" style=\"position:absolute;z-index:19;\" "+imgact+" border=0 src="+ti+">"};imar[im]=ims;}omp=mp;ofc=arg[2][0];ofb=arg[2][1];onc=arg[2][2];onb=arg[2][3];ltarg=" ";lOnfunc="";lOffunc="";ltype="";lalign=menu[8];if(!lalign)lalign="left"
	...
	if(arg[0].toLowerCase().indexOf("<img")>=0){if(menu[11]&&ns6)ns6hif=1;ipos=arg[0].toLowerCase().indexOf("<img");if(simg){SwapIM[el]=new Image();SwapIM[el].src=simg;loadWait=1;}else{SwapIM[el]=0}arg[0]=arg[0].substr(0,ipos+4)+" onclick=\"gmobj('el"+el+"').click()\" id=im"+el+" "+ arg[0].substr(ipos+5,900)}
	...
	if(ns4){
		...
		hms="";if(menu[11])hms="<div style=\";top:0;z-index:19;position:absolute;\"><div onmouseover=\"clearTimeout(Mtimer)\" id=hsep"+el+" style=\"position:absolute;background:"+sbgc+";width:"+menu[b]+"px; clip:rect(0 "+menu[b]+"px 100% 0);\">\n</div>\n</div>";
...
if(menu[19]){mt+="<div id=S"+mnu+" style=\"position:absolute;z-index:19;overflow:hidden;\">"}
	}
			...
			{				
				...
				if (!menu[11])
				{
					...
					if (hlp[11]&&mbw==1)
					{
						...
						txt+="<div style=\"border-top:1px solid "+hlp[13]+"; background:"+hlp[12]+"; width:"+dwid+"px;position:absolute;z-index:19;clip:rect(0 100% 2px 0);height:1px;\">"
						txt+="</div>\n</div>"
					}
					else
					{
						txt="<div>";
						txt+="<div onmouseout=\"popdn();\" onmouseover=\"clearTimeout(Mtimer)\" style=\"background:"+sbgc+";width:"+dwid+"px;position:absolute;z-index:19;"
...
function mOL(){if(!ns61&&ns6||ns6hif){Minit()}MLoaded=1;if(Oload)Oload();if(loadWait||opera){for(x=0;x<parr.length;x++){fixb("menu"+parr[x]);popup(parr[x])}}sis();MScan();closeallmenus()}
