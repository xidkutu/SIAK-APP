...
if ((pjbpath!=null) && (!pjbpath.trim().equals("")))
{
visible="";
}
%>
...
<SCRIPT language=JavaScript src="scripts/ktpttd.js">
</SCRIPT>
<META content="MSHTML 6.00.2800.1141" name=GENERATOR></HEAD>
<BODY leftMargin=0
topMargin=0 MARGINHEIGHT="0" MARGINWIDTH="0" bgColor=#93BEE2 text="#000000" onLoad="window.history.forward(1)">
<form name="form1" method="post" enctype="multipart/form-data" action="Update_Setup_KK_KTPADV.jsp">
...
%>
                  ...
                  class="boxdefault">
                          ...
                          <tr valign="top">
                            ...
                            <td class="C-Field" colspan="2">
                              <input
                                class=boxdefault size=27 name=jab_kec value="<%=record.getKecJabata()%>" maxlength="40" onKeyUp="return char(form1.jab_kec)" disabled>
                            </td>
                          </tr>
                          <tr valign="top">
...
document.all['nip_lrh'].value= getKelurahanItem(document.all['no_prop'].value, document.all['no_kab'].value, document.all['no_kec'].value, kelcode, 7);
						  ...
						  <tr><td colspan=4>
										...
										%>
										<input name="dokumenfile" type=file class="boxstandard"  onChange="LimitAttach(this.form, this.form.dokumenfile.value,this.form.imgdokumen)" onKeyDown="this.blur()">
										</div>
										</b></div>
										</td>
										...
										<img name="imgdokumen" src="<%=pjbpath%>" width="52" height="65" <%=visible%>>
										<script language="javascript">
										var res=null;
										if (document.all['pjbpath']!=null)
										{
										res=document.all['pjbpath'];
										if (res.length>1)
										{
										document.all['imgdokumen'].style.visibility="visible";
										}
										}
										</script>
										</div>
