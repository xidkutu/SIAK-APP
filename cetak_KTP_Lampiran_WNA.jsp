     ...
     if (window.print)
     {
          if (confirm("Apakah benar KTP ini akan dicetak ?"))
          {
            ...
            for(i=0;i<document.all.totalpage.value;i++)
            {
             ...
             window.print();
             window.opener.location="cetak_KTP_WNA.jsp?update=ok&radiobutton=" + document.all['radiobutton'].value + "&param_kk="+document.all["param_kk"].value;
             window.close();
 }   } }
...
{
%>
<div id="divZoom<%=i%>" style="position:absolute; width:200px; height:115px; z-index:1;left:409px;top:<%=top%>;<%if(totalPage==1){%>zoom:100%<%}else{%>zoom:50%<%}%>" onClick="zoom(this.id)">
