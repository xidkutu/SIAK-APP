...
function openSearchWin()
{
 if(document.all['no_kel'].value=="")
   ...
   for (i=0;i<radio.length;i++)
  {
   ...
   if (!vchecked)
   {
     alert("Belum Pilih Jenis Cetak KK!");
      return false;
   }
window2=openIT('adv_list_kk.jsp?r=h&type=4',310,140,580,475,'window2',5,false,false,false);
}
...
//End Specific NIK
   ...
   DaftarCetakKKHome home = (DaftarCetakKKHome)controller.lookup("DaftarCetakKK", DaftarCetakKKHome.class);
  daftarCetakKK = home.create();
  System.out.println("jika nama : "+spesifik_nama.trim());
  System.out.println("nama : "+spesifik_nama.trim());
   System.out.println("print : "+stat_print);
   System.out.println("stat : "+stat_change);
  vDaftarCetakKk=daftarCetakKK.getSpecifickknamabanyak(""+spesifik_nama.trim()+"",stat_print,stat_change,record.getNoProp(),
   record.getNoKab(),record.getNoKec(),no_kel);
   //vDaftarCetakKk=daftarCetakKK.getSpecifickk(""+spesifik_kk.trim()+"",stat_print,stat_change,record.getNoProp(),
   //record.getNoKab(),record.getNoKec(),no_kel);
  if(vDaftarCetakKk.size()==0 ) {
...
tot_page=(long)(Math.ceil((double)tot_rec/40));
...
<font color="#2A00AA">Tampilan Semua Data</font></a></b></div>
                            ...
                            <td width="242">
                              <div align="center"><b>
<a href="#" onClick="openSearchWin()"><font color="#2A00AA">Pencarian Canggih
</font>
</a></b></div>
                            </td>
                          </tr>
...
<%
      ...
      { System.out.println(112);
          %>
                          <tr>
                            <td class="C-Field" width="10%">
                              <div align="center">
                               <%
				try {
				if (kkObject.getNo_rt()!=null) {
                                 suno_rt=kkObject.getNo_rt();
				}
				if (suno_rt.equals("")) {
					suno_rt="-";
				}
				...
				try {
				if (kkObject.getNo_rw()!=null) {
                                suno_rw=kkObject.getNo_rw();
				}
				if (suno_rw.equals("")) {
					suno_rw="-";
				}
				...
				try {
				if (kkObject.getKode_pos()!=null) {
                                suno_pos=kkObject.getKode_pos();
				}
				if (suno_pos.equals("")) {
					suno_pos="-";
				}
				if (suno_pos.equals("0")) {
					suno_pos="-";
				}
				}
				catch (Exception delr) {
				...
				%>
                                <input type=checkbox name="cb" value="<%="*"+kkObject.getNomorKK()+"*"+kkObject.getNamaKep()+"*"+kkObject.getAlamat()+"*"+suno_rt+"*"+suno_rw+"*"+suno_pos%>">
                              </div>
                            </td>
...
%>
				...
				//System.out.println("1B.suno_rw = ["+suno_rw+"]");
				}
				if (suno_rw.equals("")) {
					suno_rw="-";
				//System.out.println("2B1.suno_rw = ["+suno_rw+"]");
				}
				if (suno_rw.equals("0")) {
					suno_rw="-";
				//System.out.println("2B2.suno_rw = ["+suno_rw+"]");
				}
				}
				catch (Exception delr) {
				suno_rw="-";
				...
				%>
                            ...
                            <td class="C-Field" width="35%">
                              <div align="center"> <%=obj.getNamaKep().toUpperCase()%></div>
                            </td>
...
<%
if ((start>40) && (vDaftarCetakKk.size()!=0))
{
%>
 <b>&lt;&lt;
