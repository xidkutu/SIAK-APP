...
%>
                                <select  name=naprop onChange="wilreset0(pilwil.nprop,pilwil.nkab,pilwil.nakab);pilwil.nprop.value=this.value;document.all['cur_naprop'].value=this.options[this.selectedIndex].text;ch_kab(pilwil.nprop.value,pilwil.nakab, 'false');" class="boxdefault">
...
%>
                                <select name=nakab onChange="pilwil.nkab.value=this.value;document.all['cur_nakab'].value=this.options[this.selectedIndex].text" class="boxdefault">
                                  <option selected value="">-- Pilih Kabupaten/Kota
                                  --</option>
                                </select>
                                <%}%>
                                <%if(level==2)
...
%>
   ...
   for(int i=2003;i<=thisYear;i++)
                              ...
                              <td>Tanggal </td>
                              <td width="10">:</td>
                              <td colspan="2"> <%=record.getToday()%> </td>
                              <%}%>
                            </tr>
                          </table>
                        </td>
...
%>
window.open("pilihwilayah?screen_code="+<%=screen_code%>+"&level=2&nprop="+pilwil.nprop.value+"&nkab="+pilwil.nkab.value+"&naprop="+document.all['cur_naprop'].value+"&nakab="+document.all['cur_nakab'].value+tahun,'','fullscreen=yes,scrollbars=yes,resizable=yes,menubar=no,toolbar=no');
}
</script>
</BODY>
</HTML>
